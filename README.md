...
*   **🖱️ Запуск в один клик (Drag-and-Drop) для Windows:** Для нетехнических пользователей создан премиальный файл запуска **`run_parser.bat`**. Просто перетащите ваш Excel-файл с номерами мышкой на иконку скрипта, программа всё сделает сама и **автоматически откроет готовый отчет** в Excel!
...
*   `input_number` — исходный номер документа из вашего входного файла.
*   `registry_type` — тип найденного документа (`certificate` — сертификат, `declaration` — декларация, `not_found` — не найден, `ambiguous` — найдено несколько совпадений, `error` — техническая ошибка поиска).
*   `registry_number` — официальный нормализованный номер документа из реестра.
*   `id` — внутренний уникальный числовой идентификатор записи.
*   `url` — **прямая кликабельная веб-ссылка** на публичную карточку документа на сайте Росаккредитации.
*   `reg_date` — дата регистрации документа (YYYY-MM-DD).
*   `end_date` — дата окончания срока действия документа.
*   `status_id` — внутренний числовой код статуса документа.
*   `status` — **расшифрованный статус документа на русском языке** (Действует, Архивный и т.д.).
*   `error` — описание ошибки, если при обработке номера произошел сбой.
